...
from keyboards.menu_inline import make_callback_data, start_menu_cd
...
async def main_keyboard():
    ...
    markup.add(
        ...
        InlineKeyboardButton(text="Творческое испытание", callback_data=filter_cd.new(subject_id='Творческое испытание')),
    )
    markup.row(
...
async def filter_direction_keyboard(list_directions, direction_id):
    ...
    markup.row(
        InlineKeyboardButton(text="Назад", callback_data=make_direction_cd(level=CURRENT_LEVEL-1, list_directions=list_directions))
    )
    return markup
